...
```python
asd = Director.objects.all()
for i in range(len(Director.objects.all())):
    print(asd[i].name, asd[i].debut, asd[i].country)
```
...
dd.append(direct.name)
dd.append(direct.debut)
dd.append(direct.country)
print(*dd)
```
...
```python
dir = Director.objects.get(country='USA')
...
dir.country = 'USA'
dir.save()
dir2 = Director.objects.get(country='USA')
print(dir2.name, dir2.debut, dir2.country)
```
...
```
get()은 값을 1개만 리턴하는데 director는 8개나 리턴하잖아!..

get() : 값이 한개 초과 또는 없으면 오류.
```
...
```python
dir = Director.objects.filter(country='KOR')
for i in range(len(dir)):
    print(dir[i].name, dir[i].debut, dir[i].country)
```

### 13. 본인이 생각하는 혹은 찾은 `get` 과 `filter` 의 차이를 작성하세요.

```
get은 찾고자 하는 객체가 2개 이상이거나 없으면 오류 (단 하나의 값이 존재하면 사용)
filter는 0개 또는 2개 이상일 경우 사용
```
...
```python
byChulSoo = Director.objects.get(name='김철수')
byChulSoo.delete()
```